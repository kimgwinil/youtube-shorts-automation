...
img = Image.new("RGB", (1024, 1536), (60, 40, 80))
...
draw.ellipse([362, 668, 662, 868], fill=(120, 220, 160))   # 중앙 주제(보존되어야 함)
img.save(src)
...
from shorts_automation.script_builder import EssayScript  # noqa: E402

font = Path(os.environ["FONT_FILE"])
script = EssayScript(
    topic="세로영상 검증", lines=["9:16 비율 자동 검증"], author_line="검증",
    source_line="CI", is_original=True, visual_style="photoreal",
    image_prompt_en="verify", bgm_prompt_en="verify", bgm_mood="calm", mood="dawn",
    title="9:16 검증", description="-", tags=["verify"],
)
result = render_short(
    script=script,
    output_dir=OUT,
